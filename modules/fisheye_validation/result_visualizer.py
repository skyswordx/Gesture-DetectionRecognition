# -*- coding: utf-8 -*-
"""
ResultVisualizer - Visualization tools for fisheye calibration validation results
"""
...
class ResultVisualizer:
    """Visualizer for fisheye calibration validation results."""
    ...
    def show_comparison(self, result: ValidationResult, title: str = "") -> None:
        """Display side-by-side comparison of original, distorted, and recovered images."""
    ...
    def show_difference_heatmap(self, result: ValidationResult) -> None:
        """Display difference heatmap highlighting pixel-level discrepancies."""
    ...
    def annotate_metrics(self, image: np.ndarray, result: ValidationResult) -> np.ndarray:
        """Annotate image with computed metrics (PSNR, SSIM, max error)."""
        ...
        for i, metric in enumerate(reversed(metrics)):
            ...
            (text_width, text_height), _ = cv2.getTextSize(
                metric, self.FONT, self.FONT_SCALE, self.FONT_THICKNESS
            )
            cv2.rectangle(annotated, (x_offset - 2, y - text_height - 2),
                         (x_offset + text_width + 2, y + 4), self.BG_COLOR, -1)
            cv2.putText(annotated, metric, (x_offset, y),
    ...
    def save_report(self, result: ValidationResult, output_dir: str) -> None:
        """Save complete validation report to specified directory."""
    ...
    def _add_label(self, image: np.ndarray, label: str) -> np.ndarray:
        ...
        cv2.rectangle(image, (5, 5), (text_width + 15, text_height + baseline + 10),
                     self.BG_COLOR, -1)
        cv2.putText(image, label, (10, text_height + 8),
                   self.FONT, self.FONT_SCALE, self.FONT_COLOR, self.FONT_THICKNESS)
        return image
